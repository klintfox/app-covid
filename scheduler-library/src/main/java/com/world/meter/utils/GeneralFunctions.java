...
public class GeneralFunctions {
	...
	/**
	 * 
	 * @param urlHtml de la p�gina que contiene una tabla
...
//			}
				...
				for (int j = 0; j < rowItems.size(); j++) {
					...
					if (flag && count == 1) {
						if (j == 0) {
							...
							country.setTotalCases(Integer.parseInt(rowItems.get(j).text().replace(",", "")));
						} else if (j == 2) {
		...
		System.out.println("Salio GET Countries HTML");
		return lstCountries;
	}
